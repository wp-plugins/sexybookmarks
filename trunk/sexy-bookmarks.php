...
function bookmark_list_item($name, $opts=array()) {
	...
	return sprintf(
		'<li class="%s"><a href="%s"%s title="%s">%s</a></li>',
		$name,
		$url,
		$sexy_plugopts['targetopt']=="_blank"?' rel="external"':'',
...
function sexy_public() {
	...
	if ($sexy_plugopts['expand'] || $sexy_plugopts['autocenter']) {
		wp_register_script('sexy-bookmarks-public-js', SEXY_PLUGPATH.'sexy-bookmarks-blank-public.js', array('jquery'), SEXY_vNum);
