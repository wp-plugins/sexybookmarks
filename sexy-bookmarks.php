...
register_activation_hook(__FILE__, 'shr_activation_hook' );
...
function shr_settings_page() {
	...
	echo '<h2 class="sexylogo">SexyBookmarks<span class="sh-logo"></span></h2>';
	...
	if($_POST['custom-mods'] == 'yes' || $sexy_plugopts['custom-mods'] == 'yes') {
		if(is_admin() === true && !is_dir(WP_CONTENT_DIR.'/sexy-mods')) {
			...
			copy($sexy_oldloc.'images/sharing-shr.png', $sexy_newloc.'images/sharing-shr.png');
		}
	}
	...
	if(isset($_POST['save_changes'])) {
		...
		// Remove settings and services that no longer exist
		if(isset($sexy_plugopts['bookmark']['sexy-designmoo'])) {
			unset($sexy_plugopts['bookmark']['sexy-designmoo']);
		}
		if(isset($sexy_plugopts['bookmark']['shr-designmoo'])) {
			unset($sexy_plugopts['bookmark']['shr-designmoo']);
		}
		if(isset($sexy_plugopts['bookmark']['sexy-blogospherenews'])) {
			unset($sexy_plugopts['bookmark']['sexy-blogospherenews']);
		}
		if(isset($sexy_plugopts['bookmark']['shr-blogospherenews'])) {
			unset($sexy_plugopts['bookmark']['shr-blogospherenews']);
		}
    if(isset($sexy_plugopts['xtrastyle'])) {
      unset($sexy_plugopts['xtrastyle']);
    }
    if(isset($sexy_plugopts['twittid'])) {
      unset($sexy_plugopts['twittid']);
    }


		// Set success message
		$status_message = __('Your changes have been saved successfully!', 'sexybookmarks');
		...
		if (!$error_message) {
			...
			if(shr_preFlight_Checks()) {
				...
				if($save_return[0] == 2 || $save_return[1] == 2) {
					...
					$sexy_custom_sprite = '';
					$status_message = __('Changes saved successfully. However, you should try to generate a custom sprite again later.', 'sexybookmarks');
				}
				elseif($save_return[0] == 1 || $save_return[1] == 1) {
					if (!is_writable(SEXY_PLUGDIR.'spritegen')) {
      ...
      if ($_POST['clearShortUrls'] || $_POST['shorty'] != $sexy_plugopts['shorty']) {
...
<form name="sexy-bookmarks" id="sexy-bookmarks" action="" method="post">
	<div id="sexy-col-left">
		<ul id="sexy-sortables">
			...
			<li>
				...
				<div class="box-mid-body" id="toggle2">
					<div class="padding">
						...
						<div id="twitter-defaults"<?php if(!in_array('shr-twitter', $sexy_plugopts['bookmark'])) { ?> class="hide"<?php } ?>>
							<h3><?php _e('Twitter Options:', 'sexybookmarks'); ?></h3>
              <p id="tweetinstructions">
                <strong><?php _e('Configuration Instructions:', 'sexybookmarks'); ?></strong><br />
                <?php _e('Using the strings <strong>${title}</strong> and <strong>${short_link}</strong> you can fully customize your tweet output.', 'sexybookmarks'); ?><br /><br />
                <strong><?php _e('Example Configurations:', 'sexybookmarks'); ?></strong><br />
              ...
              <p id="tweetoutput"><strong><?php _e('Example Tweet Output:', 'sexybookmarks'); ?></strong><br /><span></span></p>
						...
						<div id="ybuzz-defaults"<?php if(!in_array('shr-yahoobuzz', $sexy_plugopts['bookmark'])) { ?> class="hide"<?php } ?>>
						...
						<div id="twittley-defaults"<?php if(!in_array('shr-twittley', $sexy_plugopts['bookmark'])) { ?> class="hide"<?php } ?>>
					...
					<div class="padding">
						...
						<div id="bgimgs" class="<?php if(!isset($sexy_plugopts['bgimg-yes'])) { ?>hidden<?php } else { echo ''; }?>">
							<label class="share-sexy">
								<input <?php echo (($sexy_plugopts['bgimg'] == "shr")? 'checked="checked"' : ""); ?> id="bgimg-sexy" name="bgimg" type="radio" value="shr" />
							</label>
            ...
            <label><input <?php echo (($sexy_plugopts['position'] == "both")? 'checked="checked"' : ""); ?> name="position" id="position-both" type="radio" value="both" /> <?php _e('Above & Below Content', 'sexybookmarks'); ?></label>
		...
		<div class="box-right-body">
			<div class="padding">
				<ul class="infolinks">
					...
					<li><a href="http://www.shareaholic.com/tools/browser/" target="_blank"><?php _e('Shareaholic Browsers Add-ons', 'sexybookmarks'); ?></a></li>
					<li><a href="http://sexybookmarks.net/credits" target="_blank"><?php _e('Thanks &amp; Credits', 'sexybookmarks'); ?></a></li>
				</ul>
			</div>
		</div>
	</div>
...
}//closing brace for function "shr_settings_page"
...
function sexy_menu_link() {
	if (function_exists('add_options_page')) {
		$sexy_admin_page = add_options_page(__('SexyBookmarks', 'sexybookmarks'), 'SexyBookmarks', 'administrator', basename(__FILE__), 'shr_settings_page');
