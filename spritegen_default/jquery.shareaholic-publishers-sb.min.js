...
!SHR4P.locales[g]))g="en";g=typeof SHR4P.locales!=="undefined"?SHR4P.locales[g][SHR4P.tooltips_prefix_map[c]]+" "+SHR4P.tooltips_suffix[c]:"";SHR4P.ga.socialAction({serviceId:c,serviceAction:"render",link:a.link});var k=b("<li/>");k.addClass("shr-"+c).addClass("shareaholic").attr("style","margin-left:"+a.li_margin+"px !important;").bind("click",{serviceId:c,serviceAction:"share",link:a.link},SHR4P.ga.trackAction);var o={title:a.title,link:a.link,notes:a.notes,short_link:a.short_link,shortener:a.shortener,
shortener_key:a.shortener_key,v:a.api.share.v,apitype:a.api.share.apitype,apikey:a.apikey,source:a.api.share.source,template:h,service:c,ctype:a.ctype};c=="7"&&a.twitter_overlay&&!SHR4P.utils.isMobileBrowser()?k.append(b("<a/>").attr({rel:a.rel,target:a.target,"class":"external"}).attr(j,g).text(g).click(function(b){SHR4P.utils.showServiceLet(a,"twitter");b.preventDefault()})).appendTo(i):k.append(b("<a/>").attr({href:e(a.share_src+a.api.share.url,o),rel:a.rel,target:a.target,"class":"external"}).attr(j,
g).text(g)).appendTo(i);c=="309"&&k.click(function(b){SHR4P.utils.runBookmarklet(o);b.preventDefault()});if(!a.dontShowShareCount&&(c==5||c==7||c==2))h=function(a){a=a.count;if(a>0||a!="0"&&a!=0){for(var f=b("<div/>").addClass("shr-count").appendTo(i.find(".shr-"+c)),d=0;d<9;d+=1)d!=4&&f.append(b("<div>"+a+"</div>").addClass("shr-count-outline").css({left:10+d%3-1+"px",top:"-"+(11+parseInt(d/3)-1)+"px"}).attr("aria-hidden","true"));f.append(b("<div>"+a+"</div>").addClass("shr-count-center").css({left:"10px",
...
j+"</style>"));c.wrap('<div class="shr-7 shareaholic" style="float:left; '+n+' margin:0px 0px 0px 10px !important;"></div>');c.parent().removeClass("shr-7");c.addClass(i);c.addClass(h);SHR4P.debug(c);SHR4P.ga.socialAction({serviceId:7,serviceAction:"render",link:m});SHR4P.utils.isMobileBrowser()||c.click(function(a){SHR4P.ga.socialAction({serviceId:7,serviceAction:"share",link:m});SHR4P.utils.showServiceLet(p,"twitter");a.preventDefault()});c.attr("data-shr_status","1")}})}};
