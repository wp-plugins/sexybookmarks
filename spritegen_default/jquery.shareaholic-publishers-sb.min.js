...
(h(a)==1?b["promo-height"]:0):i(a)},i=function(a){return b["margin-top"]+b["bgimg-padding-top"]+(b["icon-height"]+b["icon-margin-top"])*h(a)+b["promo-height"]},k=function(a){SHR4P.debug("centering");var d=0;h(a)==1&&(SHR4P.debug("one row! do centering"),d=(a-f()-c()*b["icon-width"])/2,SHR4P.debug("returning "+d+" margin"));return d},l=function(a){SHR4P.debug("spacing");SHR4P.debug("rows: "+h(a));var d=0;h(a)==1&&(SHR4P.debug("one row! spacing icons"),d=Math.floor((a-f()-b["icon-width"]*c())/c()),
...
b.fbButtonPos.indexOf("bottom")!=-1?d.insertAfter(this):d.insertBefore(this);b.fbButtonPos.indexOf("right")!=-1&&k.css("float","right")}a(b.service.split(",")).each(function(d,c){var f="";if(c=="7")f=b.twitter_template;b.custom_link["before_"+d]&&h("before",d,b).appendTo(i);var g=b.designer_toolTips?"orig_title":"title",j=SHR4P.utils.getBrowserLocale(),j=j.split("-")[0];if(typeof SHR4P.locales!=="undefined"&&(typeof SHR4P.locales[j]=="undefined"||!SHR4P.locales[j]))j="en";j=typeof SHR4P.locales!==
"undefined"?SHR4P.locales[j][SHR4P.tooltips_prefix_map[c]]+" "+SHR4P.tooltips_suffix[c]:"";c=="7"&&b.twitter_overlay&&!SHR4P.utils.isMobileBrowser()?a("<li/>").addClass("shr-"+c).addClass("shareaholic").append(a("<a/>").attr({rel:b.rel,target:b.target,"class":"external"}).attr(g,j).text(j).click(function(a){SHR4P.utils.showServiceLet(b,"twitter");a.preventDefault()})).attr("style","margin-left:"+b.li_margin+"px !important;").appendTo(i):a("<li/>").addClass("shr-"+c).addClass("shareaholic").append(a("<a/>").attr({href:e(b.share_src+
...
a.attr("shr_size"),e=a.attr("data-shr_count")||a.attr("shr_count"),f=a.attr("data-shr_lang");if(typeof e=="undefined"||!e)e="horizontal";if((typeof c=="undefined"||!c)&&(typeof d=="undefined"||!d))c=encodeURIComponent(document.location.href),d=document.title;else{if(typeof c=="undefined"||!c)c=encodeURIComponent(document.location.href);if(typeof d=="undefined"||!d)d=""}if(typeof b=="undefined"||!b)b="standard";var h="";b=="medium"&&(h="padding-top:1px !important;");if(typeof f=="undefined"||!f)f=
"en";b=SHR4P.jQuery("<div style='float:left; "+h+" margin:0px 0px 0px 10px !important;'/>").get(0);a.replaceWith(b);SHR4P.jQuery(b).append("<div/>");b=SHR4P.jQuery(b).find(":first-child").get(0);SHR4P.jQuery('<a class="twitter-share-button" href="https://twitter.com/share"data-url="'+c+'"data-via="Shareaholic"data-text="'+d+'"data-related="Shareaholic"data-count="'+e+'"data-lang="'+f+'"data-counturl"'+c+'">Tweet</a>').appendTo(b)})},trackTweetButtonClick:function(a){a.type="tweet";console.log(a);
